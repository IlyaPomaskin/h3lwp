...
class PngEncoder {
    ...
    private fun createIHDR(width: Int, height: Int): ByteArray {
        ...
        ihdr.write(ByteBuffer.allocate(4).putInt(width).array())
        ihdr.write(ByteBuffer.allocate(4).putInt(height).array())
        ihdr.write(8) // bitDepth
        ihdr.write(3) // colorType (indexed)
    ...
    private fun writeChunk(type: String, content: ByteArray) {
        output.write(ByteBuffer.allocate(4).putInt(content.size).array())
        output.write(type.toByteArray())
        output.write(content)
        output.write(ByteBuffer.allocate(4).putInt(getCRC(type, content).toInt()).array())
    }
